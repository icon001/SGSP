...
uses
  ...
  IdUDPBase, IdUDPServer, IdUDPClient,IdSocketHandle,AdSocket, IPEdit;
...
type
  TfmNetConfig = class(TForm)
    ...
    chk_OldDevice: TRzCheckBox;
    btn_Ping: TSpeedButton;
  ...
  private
    ...
    procedure RegLanWiznet(SockNum:integer=0);  //UDP로 LAN 모듈 셋팅
...
var
  nServerMode : integer;
begin
  if RadioModeClient.Checked then nServerMode := 1
  else if RadioModeServer.Checked then nServerMode := 0
  else nServerMode := 0;
  //if ed_LLocalIP.Text <> ''  then
  G_stTCPConf := ed_LLocalIP.Text + ',' + ed_LLocalPort.Text + ',' + inttostr(nServerMode);
  //G_stTCPConf := IP_LLocalIP.Text + ',' + ed_LLocalPort.Text + ',' + inttostr(nServerMode);
...
begin
  ...
  FirstTickCount := GetTickCount + DELAYTIME; //3초 대기
...
begin
  ...
  FirstTickCount := GetTickCount + DELAYTIME + 1000; //5초 대기
  While Not bWizeNetLanRecv do
  begin
    Application.ProcessMessages;
  ...
  FirstTickCount := GetTickCount + DELAYTIME + 2000; //5초 대기
  While Not bWizeNetLanRecv do
  begin
    Application.ProcessMessages;
    if GetTickCount > FirstTickCount then Break;
  ...
  btn_LClose.Enabled := True;
  bNetConfigSet := True;
  G_bDirectConnect := True;
end;

procedure TfmNetConfig.RegLanWiznet(SockNum:integer=0);
var
  st,st2 : string;
  I : integer;
  No : integer;
begin
    ...
    begin
      ...
      wiznetData[3]:='T';
      //wiznetData[4]:='T';
      if SockNum = 0 then wiznetData[4]:='T'
      else wiznetData[4]:=inttostr(SockNum)[1];
    end else
    begin
      wiznetData[1]:='L';
      wiznetData[2]:='N';
      wiznetData[3]:='S';
      //wiznetData[4]:='V';
      if SockNum = 0 then wiznetData[4]:='V'
      else wiznetData[4]:=inttostr(SockNum)[1];
    end;
(*
    wiznetData[1]:='L';
...
*)
   ...
   st2:='';
    ...
    {Server IP}
    st2:='';
    if Edit_ServerIp.Text = '' then Edit_ServerIp.Text := '222.106.18.117';
    for I:= 0 to 3 do
    begin
      st:= FindCharCopy(Edit_ServerIp.Text,I,'.');
      //st:= FindCharCopy(IP_ServerIp.Text,I,'.');
    ...
    if SockNum = 7 then wiznetData[11] := #$00; //소켓 번호가 0 이 아니면 클라이언트 모드로 강제 변환하자.
    ...
    Checkbox_DHCP.Checked := False; //무조건 DHCP는 빼자
...
procedure TfmNetConfig.btn_PingClick(Sender: TObject);
var
  stExe:string;
begin
  stExe := 'cmd.exe /c ping.exe ' + ed_LLocalIP.Text ;//+ ' -t';
