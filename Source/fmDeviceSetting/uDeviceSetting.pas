...
begin
  ...
  begin
    if G_bApplicationTerminate then Exit;
    result := dmSocket.AlarmState_Check('00'); //Main 으로만 상태 조회 됨
    if result then Break;
  end;
...
begin
  ...
  begin
    if G_bApplicationTerminate then Exit;
    result := dmSocket.RegistDoorSystemInfo(L_stSelectEcuID,                             // 기기번호
              aDoorNo,                               // 문번호
...
begin
  ...
  begin
    showmessage('정시발보 시각은 숫자로 입력 하셔야 합니다.');
    Exit;
  end;
  if Not IsDigit(ed_cccStartTime.Text) then
  begin
    showmessage('정시발보 실시 시간은 숫자로 입력 하셔야 합니다.');
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더1');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더2');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더3');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더4');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더5');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더6');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더7');
      ...
      begin
        ...
        begin
          bNode:= TreeView_Device.Items.AddChild(aNode,aEcuID + ':카드리더8');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          bNode.StateIndex:= -1;
          cNode:= aTreeView.Items.AddChild(bNode,FindField('ECU_ID').AsString + ':방범');
          ...
          if FindField('DOOR_TYPE1').AsString <> '0' then  //방범전용이 아니면
          begin
            cNode:= aTreeView.Items.AddChild(bNode,FindField('ECU_ID').AsString + ':출입문1');
            if UpperCase(FindField('DOORMODE_TYPE1').AsString) = '1' then //개방모드
            begin
          ...
          if FindField('DOOR_TYPE2').AsString <> '0' then  //방범전용이 아니면
          begin
            cNode:= aTreeView.Items.AddChild(bNode,FindField('ECU_ID').AsString + ':출입문2');
            if UpperCase(FindField('DOORMODE_TYPE2').AsString) = '1' then //개방모드
            begin
              if UpperCase(FindField('LASTDOOR1STATE').AsString) = 'C' then
              begin
                cNode.ImageIndex:=5;
